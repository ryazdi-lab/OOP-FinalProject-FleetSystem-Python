...
class Truck(Vehicle, Rentable, Insurable):
    def calculate_daily_cost(self) -> float:
        return self.base_daily_rate *1.5

    def get_vehicle_type(self) -> str:
        return "truck"
    def perform_maintenance(self) -> None:
        print(f" تعمیرات کامیون انجام شد{self.brand}")
    def calculate_rent(self, days: int) -> float:
        return self.calculate_daily_cost() * days
    def reserve(self) -> None:
        print(f"کامیون رزرو شد {self.brand}")
    def return_vehicle(self, actual_days: int) -> None:
        print(f" کامیون{self.brand} بعد از {actual_days} روز برگردونده شد.")
